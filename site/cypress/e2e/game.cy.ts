...
describe('Game', () => {
     ...
     *   never hits the real server in CI.  In local dev the stub is still
    ...
    beforeEach(() => {
        ...
        cy.intercept('POST', '/api/round', (req) => {
            ...
            expect(Array.isArray(req.body.bets)).to.equal(true);
    ...
    it('registers on Play and logs out on Home', () => {
        cy.get('[data-cy="nickname-enter-form"]').clear().type('cypress');
